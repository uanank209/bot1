...
exports.run = function(o) {
  ...
  //if (!["100033478361032" ].includes(o.event.senderID))return send(`[⚜️] ➜ Bạn không đủ quyền hạn sử dụng lệnh này!`);
  
  switch (o.args[0]) {
    case 'add': {
      if (!o.args[1])return send(`[⚜️] ➜ Sử dụng {prefix}{command name} add {ID thread} {ID user} {time start} => {time end}`);
      ...
      send(`[⚜️] ➜ Đã thêm ID vào danh sách thuê bot.`);
    };
      break;
    case 'list': {
      send(`=== 『 DANH SÁCH THUÊ BOT 』 ===\n━━━━━━━━━━━━━━━━\n\n${data.map(($, i)=>`${i+1}. ${global.data.userName.get($.id)}\n[📝] ➜ Tình trạng: ${new Date(form_mm_dd_yyyy($.time_end)).getTime() >= Date.now()+25200000?'Chưa Hết Hạn ✅': 'Đã Hết Hạn ❎'}\n[🌾] ➜ Nhóm: ${(global.data.threadInfo.get($.t_id) || {}).threadName}`).join('\n━━━━━━━━━━━━━━━━\n')}\n\n[⚜️] ➜ Reply STT để xem chi tiết.\n[⚜️] ➜ Reply del STT để xóa khỏi danh sách.\n[⚜️] ➜ Reply out STT để thoát nhóm (cách nhau để chọn nhiều số)\n[⚜️] ➜ Reply giahan STT {time_start} => {time_end}`, (err, res)=>(res.name = exports.config.name, res.event = o.event, res.data = data, global.client.handleReply.push(res)));
    };
      break;

    default: send(`=== 『 SUPPORT 』 ===\n━━━━━━━━━━━━━━━━\n[⚜️] ➜ Dùng: ${global.config.PREFIX}${this.config.name} add → Để thêm nhóm vào danh sách thuê bot\n[⚜️] ➜ Dùng: ${global.config.PREFIX}${this.config.name} list → Để xem danh sách thuê bot\n[⚜️] ➜ 𝗛𝗗𝗦𝗗 → ${global.config.PREFIX}${this.config.name} lệnh cần dùng.`)
      break;
  }
  save();
};
exports.handleReply = async function(o) {
  ...
  if (isFinite(o.event.args[0])) {
    ...
    if (!info)return send(`[⚜️] ➜ Số thứ tự không tồn tại!`);
    ...
    return send(`=== 『 THÔNG TIN THUÊ BOT 』 ===\n━━━━━━━━━━━━━━━━\n[👤] ➜ Tên người thuê: ${global.data.userName.get(info.id)}\n[🌐] ➜ link Facebook: https://www.facebook.com/profile.php?id=${info.id}\n[👥] ➜ Nhóm: ${(global.data.threadInfo.get(info.t_id) || {}).threadName}\n[⚜️] ➜ ID Nhóm: ${info.t_id}\n[📆] ➜ Ngày Thuê: ${info.time_start}\n[⏳] ➜ Hết Hạn: ${info.time_end}\n[📌] ➜ Còn ${(()=> {
      ...
      return `[⚜️] ➜ ${days} ngày ${hour} giờ là hết hạn.`;
    })()}`);
    ...
    if (!data[STT-1])return send(`[⚜️] ➜ Số thứ tự không tồn tại`);
